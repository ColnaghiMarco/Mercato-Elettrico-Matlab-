...
% 2-Eliminazione del rumore con Polinomio KERNEL e creazione serie storiche
% con i residui del kernel
...
% Estengo la serie temporale (altrimenti non potrei applicare il kernel ai
% primi e gli ultimi valor della serie)
...
for col = 1:width(y_Ore)
    kernel_data = y_Ore{:, col}; % Estrai i dati della colonna corrente
    
    % Applica la convoluzione tra i dati e il kernel gaussiano
    smoothed_data = conv(kernel_data, gaussian_kernel, 'same');

    residues{:, col} = kernel_data - smoothed_data;
    ...
    title(['Serie Temporale Denoised con Kernel Smoothing (Kernel Gaussiano) (', y_Ore.Properties.VariableNames{col}, ')']);
    xlabel('Tempo');
    ylabel('Valori');
    
    subplot(4,1,3);
    plot(t, [kernel_data, smoothed_data]);
    title(['Confronto Serie e Approssimazione Kernel (', y_Ore.Properties.VariableNames{col}, ')']);
    xlabel('Tempo');
    ylabel('Valori');
