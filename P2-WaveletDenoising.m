...
for col = 1:width(y_Ore)
    ...
    % Decomposizione con wavelet
    [C, L] = wavedec(kernel_data, level, wname);
    
    % Thresholding dei coefficienti di wavelet
    threshold = 3 * median(abs(C)) / 0.6745; % Soglia universale di thresholding
    C_denoised = wthresh(C, 's', threshold);
    
    % Ricostruzione del segnale denoised
    smoothed_data = waverec(C_denoised, L, wname);
    ...
    title(['Serie Temporale Denoised con Wavelet (', y_Ore.Properties.VariableNames{col}, ')']);
    xlabel('Tempo');
    ylabel('Valori');
    
    subplot(4,1,3);
    plot(t, [kernel_data, smoothed_data]);
    title(['Confronto Serie e Approssimazione Wavelet (', y_Ore.Properties.VariableNames{col}, ')']);
    xlabel('Tempo');
    ylabel('Valori');
