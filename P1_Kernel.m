...
%% 2-Applicazione dello smoother (kernel)

% Specifica il sigma del kernel gaussiano
sigma = 30; % Sostituisci con il valore desiderato
kernel_size = 0.5 * sigma; 
x = -kernel_size:kernel_size;

% Crea il kernel gaussiano
gaussian_kernel = exp(-x.^2 / (2 * sigma^2));
gaussian_kernel = gaussian_kernel / sum(gaussian_kernel); % Normalizza il kernel
...
% Funzione per fare il padding riflesso
reflectPadding = @(data, padSize) [flipud(data(1:padSize)); data; flipud(data(end-padSize+1:end))];

% Ciclo per ogni colonna di y_Ore
kernel_data = y_PUN; % Estrai i dati della colonna corrente
    
% Calcola la dimensione del padding
pad_size = floor(kernel_size);

% Applica il padding riflesso
padded_data = reflectPadding(kernel_data, pad_size);

% Applica la convoluzione tra i dati con padding e il kernel gaussiano
smoothed_data = conv(padded_data, gaussian_kernel, 'same');

% Rimuovi il padding dai dati lisciati
smoothed_data = smoothed_data(pad_size+1:end-pad_size);

% Salva i dati smussati nella tabella trend_data
trend_data = smoothed_data;

% Calcola i residui
residues = kernel_data - smoothed_data;
...
for i=1:n_predictions
    ...
    % Applica padding riflesso prima della convoluzione
    pad_size = floor(kernel_size); % Calcola la dimensione del padding
    reflectPadding = @(data, padSize) [flipud(data(1:padSize)); data; flipud(data(end-padSize+1:end))];
    Y_dati_storici_padded = reflectPadding(Y_dati_storici, pad_size);
    smoothed_forecast_padded = conv(Y_dati_storici_padded, gaussian_kernel, 'same');
    smoothed_forecast = smoothed_forecast_padded(pad_size+1:end-pad_size);;
